...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libjulia_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("libjulia_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "libjulia_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"libjulia", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "libjulia_jll", "src", "libjulia_jll.jl"))
        @info("libjulia_ll dev'ed out to /depot/dev/libjulia_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("5ad3ddd2-0711-543a-b040-befd59781bbf"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libjulia", artifacts_toml) for e in artifacts["libjulia"]]
...
if best_platform === nothing
    @debug("Unable to load libjulia; unsupported platform $(triplet(platform_key_abi()))")
else
